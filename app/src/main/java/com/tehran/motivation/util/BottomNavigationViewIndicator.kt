...
import com.tehran.motivation.R.styleable
...
class BottomNavigationViewIndicator @JvmOverloads constructor(
    ...
    init {
        if (attrs == null) {
            ...
            with(context.obtainStyledAttributes(attrs, styleable.BottomNavigationViewIndicator)) {
                targetId =
                    getResourceId(
                        styleable.BottomNavigationViewIndicator_targetBottomNavigation,
                        NO_ID
                    )
                val clipableId =
                    getResourceId(styleable.BottomNavigationViewIndicator_clipableBackground, NO_ID)
                backgroundDrawable = if (clipableId != NO_ID) {
                    getDrawable(context, clipableId) ?: ColorDrawable(Color.TRANSPARENT)
                } else {
                    ColorDrawable(
                        getColor(
                            styleable.BottomNavigationViewIndicator_clipableBackground,
                            Color.TRANSPARENT
                        )
                    )
                }
    ...
    override fun onAttachedToWindow() {
        ...
        index = target?.childCount?.minus(1) ?: 0
